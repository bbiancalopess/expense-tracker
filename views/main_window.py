...
class MainWindow(tk.Tk):
    ...
    def configure_style(self):
        ...
        style.map(
            "Sidebar.TButton", background=[("active", self.color_palette["dark_blue"])]
        )
        ...
        style.map("TButton", background=[("active", self.color_palette["dark_blue"])])
        ...
        style.map(
            "Exit.TButton", background=[("active", self.color_palette["dark_red"])]
        )
    ...
    def create_widgets(self):
        ...
        for text, command in buttons:
            btn = ttk.Button(
                sidebar, text=text, command=command, style="Sidebar.TButton"
            )
            btn.pack(pady=5, padx=10, fill="x")

        exit_btn = ttk.Button(
            sidebar, text="Sair", command=self.quit, style="Exit.TButton"
        )
        exit_btn.pack(side="bottom", pady=10, padx=10, fill="x")

        # Frame de conteúdo principal que será trocado
        self.content_frame = tk.Frame(
            main_container, bg=self.color_palette["light_gray"]
        )
    ...
    def show_home(self):
        ...
        self.transactions_panel = TransactionsPanel(
            self.content_frame, self.color_palette
        )
    ...
    def open_add_transaction(self):
        AddTransactionWindow(
            master=self, callback=self.transactions_panel.refresh_transactions
        )
