...
class MetricsWindow(tk.Frame):
    ...
    def create_widgets(self):
        ...
        ttk.Radiobutton(toggle_frame, text="Por Categoria", variable=self.selected_view,
                        value="categoria", style="TRadiobutton", command=self.update_view).pack(side="left", padx=5)
        ttk.Radiobutton(toggle_frame, text="Por Mês", variable=self.selected_view,
                        value="mes", style="TRadiobutton", command=self.update_view).pack(side="left", padx=5)

        # Área de métricas
        self.metrics_frame = tk.Frame(main_frame, bg=self.color_palette["white"], padx=20, pady=15)
